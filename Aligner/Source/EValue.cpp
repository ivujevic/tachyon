...
std::vector<EValueConstants> kEValueConstants = {{-1, -1, 0.3176, 0.134, 0.4012, 0.7916, 0.623757, 4.964660, 4.964660},
...
EValue::EValue(uint64_t database_cells, ScoreMatrix& scorer) {
    ...
    auto gap_open = scorer.gapOpen_;
    auto gap_extend = scorer.gapExtend_;

    //if (scorer->type() == ScoreMatrixType::kBlosum62) {
    for (uint32_t i = 0; i < kEValueConstants.size(); ++i) {
        if (gap_open == kEValueConstants[i].gap_open && gap_extend == kEValueConstants[i].gap_extend) {
            index = i;
            break;
        }
    }
...
double EValue::calculate(int32_t score, uint32_t query_length, uint32_t target_length) const {
