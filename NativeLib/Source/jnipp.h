...
#include <android/log.h>
        #define JNIPP_ASSERT(condition, message, ...) if (!(condition)) __android_log_print(ANDROID_LOG_ERROR, "JNI", message, ##__VA_ARGS__);
    #else
...
public:
    ...
    static LocalRef<String> create(const std::string_view& value);
...
public:
    ...
    // disallow use of []
private:
    void operator[](int ) const { // !!! use (*ref)[idx]
    }
public:
...
public:
    ...
    jmethodID getMethodID() const {
        if (_methodID == nullptr) {
            ...
            JNIPP_ASSERT(cls, "Method: class %s not found", _clsName);
            jmethodID res = env->GetMethodID(cls, _name, _signature);
            JNIPP_ASSERT(res, "Method: method %s %s not found", _name, _signature);
            _methodID = res;
        }
        return _methodID;
    }
...
public:
    ...
    jmethodID getMethodID() const {
        if (_methodID == nullptr) {
            JNIEnv* env = Env::get();
            jclass cls = _cls ? _cls : env->FindClass(_clsName);
            JNIPP_ASSERT(cls, "NonvirtualMethod: class %s not found", _clsName);
            jmethodID res = env->GetMethodID(cls, _name, _signature);
            JNIPP_ASSERT(res, "NonvirtualMethod: method %s %s not found", _name, _signature);
            _methodID = res;
        }
        return _methodID;
    }
...
public:
    ...
    jmethodID getMethodID() const {
        if (_methodID == nullptr) {
            ...
            JNIPP_ASSERT(res, "StaticMethod: method %s %s not found", _name, _signature);
            _methodID = res;
        }
        return _methodID;
    }
    jclass getClass() const {
        JNIEnv* env = Env::get();
        jclass cls = _cls ? _cls : env->FindClass(_clsName);
        JNIPP_ASSERT(cls, "StaticMethod: class %s not found", _clsName);
        return cls;
    }
    operator jmethodID() const {
        return getMethodID();
...
public:
    ...
    jmethodID getMethodID() const {
        if (_methodID == nullptr) {
            ...
            JNIPP_ASSERT(res, "Constructor: method <init> %s not found", _signature);
            _methodID = res;
        }
        return _methodID;
    }
    jclass getClass() const {
        JNIEnv* env = Env::get();
        jclass cls = _cls ? _cls : env->FindClass(_clsName);
        JNIPP_ASSERT(cls, "Constructor: class %s not found", _clsName);
        return cls;
    }
    operator jmethodID() const {
        return getMethodID();
...
inline LocalRef<String> String::create(const std::string_view& value) {
    return LocalRef<String>::use( Env::get()->NewStringUTF(value.data()) );
}
